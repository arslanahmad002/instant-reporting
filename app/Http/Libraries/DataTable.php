...
class DataTable{
    ...
    private static function groupActions() {
        $customActionType = Request::get('customActionType');
        $customActionName = Request::get('customActionName');
        $customActionId = Request::get('customActionId', []);
    ...
    public static function get($where = array(), $joins = array(), $extraColumns = array(), $misc = array()) {
        self::$_length = intval(\request()->get('length'));
        if (self::$_length <= 0) {
            self::$_length = 10;
        }

        if(\request()->get('draw',0) > 1){
//            self::$_start = (\request()->get('draw') * self::$_length) - self::$_length;
            self::$_start = \request()->get('start') ;
        }else{
            self::$_start = 0;
        }
        self::$_order = \request()->sort;
        self::$_requestColumns = Request::get('columns', []);
        self::$_search = Request::get('search');
        self::$_draw = \request()->get('draw',1);
        ...
        return array(
                ...
                "recordsTotal" => intval($pages),
                "recordsFiltered" => intval($totalPages),
                "perpage" => $perPage,
                "start" => self::$_start,
    ...
    public static function whereJsonContains($column,$value){
        self::$_query->whereJsonContains($column,$value);
    }
    ...
    private static function dataOutput() {
        ...
        for ($i = 0, $ien = count($data); $i < $ien; $i++) {
            ...
            for ($j = 0, $jen = count(self::$_columns); $j < $jen; $j++) {
                ...
                if (isset(self::$_columns[$j]['is_raw'])){
                    $fieldName = self::$_columns[$j]['dt'];
                }
